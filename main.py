...
def replace_files_in_folder():
    ...
    source_folder_name = "PLACE YOUR CUSTOM FONT IN HERE"
    source_folder_path = os.path.join(script_dir, source_folder_name)

    # Validate the source folder's existence
    if not os.path.isdir(source_folder_path):
        print(f"\nError: The source folder was not found.")
        print(f"Please create a folder named '{source_folder_name}' in the same directory as the script.")
        print(f"Expected to find it at: {source_folder_path}")
        return
